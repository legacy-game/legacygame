...
{
    public sealed class AcceptJobCommand : IWorldCommand
    {
        ...
        public WorldCommandResult Execute(WorldCommandContext context)
        {
            ...
            if (!context.State.TryGetPlace(_workplacePlaceId, out PlaceState place)) {
                return WorldCommandResult.Failure($"Workplace not found: {_workplacePlaceId}");
            }
            ...
            if (roleSystem.HasActiveRole(citizen.Id, role.Id, place.Id)) {
                return WorldCommandResult.Success($"{citizen.DisplayName} already has the {role.DisplayName} job at {place.DisplayName}.");
            }
            ...
            HistoryEvent historyEvent = context.History.Create(
                ...
                HistoryEventKind.JobAccepted,
                $"{citizen.DisplayName} accepted the {role.DisplayName} job at {place.DisplayName}.",
                new[] { citizen.Id },
                new[] { place.Id });

            return WorldCommandResult
                .Success($"{citizen.DisplayName} accepted the {role.DisplayName} job at {place.DisplayName}.")
