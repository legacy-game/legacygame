...
namespace Legacy.Commands
{
    public sealed class DoWorldActionCommand : IWorldCommand
    {
        ...
        public WorldCommandResult Execute(WorldCommandContext context)
        {
            ...
            if (IsQueuedJobAction(_action)) {
                if (!context.State.TryGetWorkplaceByPlace(place.Id, out WorkplaceState workplace)) {
                    return WorldCommandResult.Failure($"No workplace is attached to {place.DisplayName}.");
                }

                return context.State.TryGetNextQueuedTask(workplace.Id, _action, out JobTaskState _)
                    ? WorldCommandResult.Failure("Use the workplace counter while on an active shift to start this queued task.")
                    : WorldCommandResult.Failure("No queued work is waiting here. Wait for a customer or delivery.");
            }
        ...
        private static bool IsQueuedJobAction(WorldActionKind action)
        {
            return action == WorldActionKind.ServeCustomer || action == WorldActionKind.StockShelves;
        }
    }
}
